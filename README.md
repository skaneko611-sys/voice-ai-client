...
## 2回目以降の起動（Windows・ワンクリック起動）

`start.bat` をダブルクリックするだけで、サーバー起動とChromeでのページ表示を自動的に行います。
（`.env`の設定は初回のみでOKです。2回目以降は不要です。）

1. エクスプローラーでプロジェクトフォルダを開く
2. `start.bat` をダブルクリック
3. サーバー用のコマンドプロンプトが立ち上がり、数秒後に自動的にChromeが開きます
4. Chromeが開くと、ボタンを押さなくても**自動的に通話が開始**され、そのまま話しかけられます

デスクトップから起動したい場合は、`start.bat` を右クリックして「ショートカットの作成」を選び、
できたショートカットをデスクトップに移動しておくと便利です。

サーバー用のコマンドプロンプトのウィンドウは、使い終わったら閉じてください（閉じるとサーバーが停止します）。

### 自動開始の仕組みと初回のマイク許可について

`start.bat` は、このアプリ専用のChromeプロファイル（プロジェクトフォルダ内の `chrome-profile`）で、
音声の自動再生制限を無効にした状態でChromeを起動します。これにより、ページを開いた瞬間に
通話が始まり、AIの声もクリックなしでそのまま再生されます。

**普段お使いのChromeとは別のプロファイル（別人格）として動作するため、初回起動時のみ
マイクの使用許可ポップアップが再度表示されます。** そこで「許可」を選択してください。
2回目以降は同じプロファイル（`chrome-profile`フォルダ）を使い続けるため、許可は不要になります。
